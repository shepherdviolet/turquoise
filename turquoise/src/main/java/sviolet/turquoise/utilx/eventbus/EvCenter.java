...
class EvCenter {
    ...
    List<EvMessage> withdraw(Class<? extends EvMessage> messageType){

        //根据时间倒叙排列， 第一个为最新的消息
        TreeMap<Long, EvMessage> map = new TreeMap<>(comparator);
        //遍历所有station拉取消息
        for (EvStation station : ConcurrentUtils.getSnapShot(stations)){
    ...
    void store(Activity activity, EvMessage message){
        //获得station
        LifeCycle component = LifeCycleUtils.getComponent(activity, COMPONENT_ID);
        if (!(component instanceof EvStation)){
            try{
    ...
    void store(Fragment fragment, EvMessage message){
        //获得station
        LifeCycle component = LifeCycleUtils.getComponent(fragment, COMPONENT_ID);
        if (!(component instanceof EvStation)){
            try{
    ...
    void store(android.support.v4.app.FragmentActivity activity, EvMessage message){
        //获得station
        LifeCycle component = LifeCycleUtils.getComponent(activity, COMPONENT_ID);
        if (!(component instanceof EvStation)){
            try{
    ...
    void store(android.support.v4.app.Fragment fragment, EvMessage message){
        //获得station
        LifeCycle component = LifeCycleUtils.getComponent(fragment, COMPONENT_ID);
        if (!(component instanceof EvStation)){
            try{
