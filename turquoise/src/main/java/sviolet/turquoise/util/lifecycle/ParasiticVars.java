...
package sviolet.turquoise.util.lifecycle;
...
import sviolet.turquoise.util.lifecycle.listener.Destroyable;
 ...
 * 方法用于销毁实例, 对象可能在下一次系统GC时被销毁, 若变量被其他对象强引用, 则不会被GC. <p/>
 *
 * *************************************************************<p/>
 *
 * host 宿主 : <br/>
 * 不可为空,宿主被系统GC时,其名下所有变量会被释放引用,实现{@link Destroyable}接口的变量会调用onDestroy()方法销毁<p/>
 *
 * key 变量名 : <br/>
 * 不可为空,用于区分相同宿主名下的不同变量,相同宿主名下,若变量名重复,原变量实例会被销毁({@link Destroyable#onDestroy()})<p/>
 *
 * param 变量 : <br/>
 * 生命周期依附于宿主的变量,当宿主被系统GC时,变量也会被释放引用,实现{@link Destroyable}接口的变量会调用onDestroy()方法销毁.
 * 变量实例可能在下一次系统GC时销毁, 若变量被其他对象强引用, 则不会被GC<p/>
 *
